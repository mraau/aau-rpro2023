...
void programMovements(){
  for (int i=0; i<sizeA; i++){
    ...
    commandArray[i][0] = chosenCommand;
    while (stage == 1){
      selectParameter();
      delay(50);
    }
    commandArray[i][1] = movementParameter;
    stage = 0;
    chosenCommand =0;
    movementParameter = 0;
  }
...
void executeProgram(){
  for (int i=0;i<sizeA; i++){
    chosenCommand = commandArray[i][0];
    movementParameter = commandArray[i][1];
    printLCD("C: " + (String)commandArray[i][0], "P: " + (String)commandArray[i][1]);
    executeMovement();
    delay(500);
  }
  stage =2;
